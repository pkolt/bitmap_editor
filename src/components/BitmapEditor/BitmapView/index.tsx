...
interface BitmapViewProps {
  ...
  isArea?: boolean;
  area?: BitmapArea;
  onChangeArea?: (value: BitmapArea) => void;
}
...
export const BitmapView = ({
  ...
  const isSelectingArea = isArea && (!area || area instanceof Point);
  ...
  const handleClick = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      if (canvas && sizes && onChangeBitmap) {
        ...
        if (x >= 0 && y >= 0) {
          ...
          if (onChangeArea && isArea && !(area instanceof Area)) {
            const point = new Point(posX, posY);
            const nextSelectedArea = area instanceof Point ? new Area(area, point) : point;
            onChangeArea(nextSelectedArea);
          } else {
            if (isArea && area instanceof Area) {
              const isIntersection = area.isIntersect(new Point(posX, posY));
              if (isIntersection) {
                const nextBitmap = bitmap.clone();
                nextBitmap.setPixelValue(new Point(posX, posY), !isClear);
    ...
    [canvas, sizes, onChangeBitmap, onChangeArea, isArea, area, bitmap, isClear],
  );
  ...
  useEffect(() => {
    ...
    drawArea(ctx, sizes, !!isArea, area);
  }, [isArea, bitmap, ctx, grid, sizes, area]);
